  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    canvas{
      ...
      height: calc(100vh - 170px); /* phần còn lại cho lời chúc */
      display:block;
      border-radius: 20px;
...
<body>
  <div class="wrap">
    ...
    <div class="msg">
Merry Christmas 🎄 
Dù gì hôm nay cũng là ngày sinh nhật của Chúa nên em sẽ gửi anh một cái ôm thật ấm (dù chỉ là tưởng tượng) 
và một điều ước: Mong anh luôn vui vẻ và cười thật nhiều nha ^^    </div>
  </div>
  ...
  <script>
    ...
    // DPR scale để nét hơn
    function resize(){
      const dpr = Math.max(1, Math.floor(window.devicePixelRatio || 1));
      const rect = canvas.getBoundingClientRect();
      canvas.width  = Math.floor(rect.width * dpr);
      canvas.height = Math.floor(rect.height * dpr);
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0); // vẽ theo CSS px
    }
    ...
    function initLights(){
      ...
      // vùng cây thông (tam giác lớn)
      const topY = h * 0.08;
      const baseY = h * 0.86;
      const centerX = w * 0.5;
      const baseHalf = Math.min(w*0.35, 330);

      const count = Math.floor(90 + w/8); // càng rộng càng nhiều đèn
      for(let i=0;i<count;i++){
        // random điểm trong tam giác bằng kỹ thuật barycentric
    ...
    function drawTree(w,h,t){
      ...
      // tán cây nhiều tầng để dày và "to"
      const layers = 6;
    ...
    // nếu resize thì re-init cho đẹp
